---
title: Welcome to (OS)2G!
sub_title: Operating Systems & Open Source Group
date: Jan 22 2026
theme:
  name: tokyonight-storm # terminal-dark
  override:
    footer:
      style: template
      center: '<span style="color:grey">(OS)2G</span>'
      right: '{current_slide} / {total_slides}'
---

Welcome to (OS)2G!
===

<!--column_layout: [1, 1]-->

<!--column: 0-->

TODO OS2G image

TODO Tux

<!--column: 1-->

# What is os2g?

- The Operating Systems & Open Source Group
- Computer science social club
- Focuses on Linux and Open-Source software

# What do you do?

- Give talks on technical topics
  - Computer hardware
  - Cybersecurity
  - Server administration
  - ...and more!
- Host events and social days

Meetings
===

We have 3 types of meetings:

<!--column_layout: [1,1,1]-->

<!--column: 0-->

<!--pause-->

# Talk days

<!--column: 1-->

<!--pause-->

# Events

<!--column: 2-->

<!--pause-->

# Social days

Introductions
===

<!--column_layout: [1, 1, 1]-->

<!--column: 0-->

<!--pause-->

![](./images/carston.jpg)

# Carston

## President

## Computer & Electrical Engineering

<!--column: 1-->

<!--pause-->

![](./images/alex.jpg)

# Alex

## Primary Programmer

## Computer Science

<!--column: 2-->

<!--pause-->

![](./images/cale.jpg)

# Cale

## Treasurer

## Computer Science

What is...
===

<!--column_layout: [1,1]-->

<!--column: 0-->

# Linux?

<!--column: 1-->

# FOSS?

<!--column: 0-->

<!--incremental_lists: true-->

- A family of Operating Systems (like Windows or macOS)
  - <span style="color:#555555">yes i know its gnu/linux...</span>
- Less of a walled garden
- More of a ship you can sink at will
  - But not as scary as people make it out to be
  - Easy to dual-boot

## Practical applications

- Great environment for coding
- Universial standard for servers
- Makes later comp sci classes easier
- Computer necromancy

<!--column: 1-->

- Free and Open Source Software
- Backbone of the modern computer
  - ffmpeg
  - Git
  - Apache HTTP Server
  - Docker
  - Node.js
  - Linux itself
- Drives innovation
- Betters the experiences of everyone

## What we can do about it

- Contribute!
  - "Open-Source" means anyone can
- FOSS Jam

Upcoming Events
===

<!--column_layout: [1,1]-->

<!--column: 0-->

## Nvolvement Fair

- Next week (Jan 29)
- 4-6:30 PM @ the Union
- Feel free to stop by our booth!

## Install-a-thon

- The following week (Feb 05)
- Install linux
  - Raspberry Pi Zero W
  - Dual-boot
  - Wipe your laptop in the middle of the semester!

### Later on...

- Microtalks
- Mana Games
- FOSS Jam

<!--column: 1-->

# Join our Discord!

![](./images/discord_qr.png)

**https://os2g.club/redirect/discord**

## View the full schedule

**https://os2g.club/meetings**

<!--end_slide-->

<!--jump_to_middle-->

Any Questions?
===
